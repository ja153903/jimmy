...
Transcripts live at `~/.claude/projects/<slug>/<uuid>.jsonl`, where `<slug>` is the absolute project path with each "/" turned into "-" (so `/Users/you/proj` becomes `-Users-you-proj`). Every line is one chat message.
