...
Launch all reviewers in a single message using the Agent tool. Use the `interrogate reviewers` list from `~/.claude/jimmy-models.md` when present, one reviewer per entry, extending or shrinking the Reviewer A/B/C/D labels below to the configured entry count; otherwise use the table defaults.
