...
- `subagent_type`: your configured how-explorer role agent (default `jimmy:worker-fast`)
...
Spawn a single subagent with the `Agent` tool that explores and explains in one pass:

- `subagent_type`: your configured how-explainer role agent (default `jimmy:worker-deep`)
...
Once all explorers return, spawn a single subagent with the `Agent` tool to synthesize their findings into one coherent explanation:
...
After the explanation is complete, spawn one architectural critic per role agent in your configured how-critics list (defaults `jimmy:critic-risk`, `jimmy:critic-deep`, `jimmy:critic-broad`, `jimmy:critic-fast`), all in a single message.

For each critic:
- `subagent_type`: one role agent from the configured how-critics list.
